...
let colors = ["#ffffff", "#ffbe0b", "#fb5607", "#ff006e", "#8338ec", "#3a86ff"];
...
class VectorField {
  constructor(totalNum, canvasShader) {
    ...
    for (let i = 0; i < wn; i++) {
      for (let n = 0; n < hn; n++) {
        ...
        let dirx =
          noise(x * this.scale, y * this.scale, curTime * this.speed) - 0.5;
        let diry =
          noise(
  ...
  update() {
    ...
    for (let i = 0; i < this.directions.length; i++) {
      ...
      let dirx =
        noise(pos.x * this.scale, pos.y * this.scale, curTime * this.speed) -
        0.5;
      let diry =
        noise(
          pos.x * this.scale + this.shiftx,
          pos.y * this.scale + this.shifty,
          curTime * this.speed
        ) - 0.5;
      let dir = createVector(dirx, diry, 0);
      dir.normalize();
    ...
    if (frameCount % this.splatterframe == 0) {
      ...
      let t = pow(
        map(frameCount % this.splatterframe, 0, this.splattertime, 0.0, 1.0),
        0.5
      );
